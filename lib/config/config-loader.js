...
class ConfigLoader {
	...
	getCachedConfigArrayForPath(fileOrDirPath) {
		...
		if (
			configFilePathInfo instanceof Promise ||
			typeof configFilePathInfo.then === "function"
		) {
			throw new Error(
		...
		if (
			!configArray ||
			configArray instanceof Promise ||
			("then" in configArray && typeof configArray.then === "function")
		) {
			throw new Error(
