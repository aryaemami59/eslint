    ...
    if (isTS) {
        ...
        if (loadedTSFile === unableToLoadTSFile) {
            try {
                ...
                throw new Error("'tsx' is required for loading TypeScript configuration files. Make sure to install it.");
            }

            try {
                const { tsImport } = await import("tsx/esm/api");

                const loadedFile = await tsImport(fileURL.href, { parentURL: __filename, tsconfig: false });

                config = loadedFile?.default ?? loadedFile;

            } catch (error) {
                debug(error);

                if (error instanceof SyntaxError && (error.message.includes("import") || error.message.includes("export"))) {
                    const { require } = await import("tsx/cjs/api");

                    const loadedFile = require(filePath, __filename);
 ...
 * @param {import("../../tests/fixtures/ts-config-files/helper").FlatConfig} config The config for the file that generated the message.
